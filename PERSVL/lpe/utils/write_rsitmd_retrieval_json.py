...
def make_arrow(root, dataset_root):
    with open(f"{root}/dataset_RSITMD.json", "r") as fp:
    ...
    for cap in tqdm(captions):
        ...
        iid2imgid[filename] = "rsitmd_" + str(cap["imgid"])
        for c in cap["sentences"]:
            iid2captions[filename].append(c["raw"])

    paths = list(glob(f"{root}/images/*.tif"))
...
if __name__ == '__main__':
    dir = r'/home/tangxu/HDB/datasets/RSITMD/'
    make_arrow(dir + 'root', dir + 'ALBEF_root')