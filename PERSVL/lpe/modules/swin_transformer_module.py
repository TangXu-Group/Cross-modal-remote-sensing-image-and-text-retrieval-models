...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.depths = config.MODEL.SWIN.DEPTHS
        self.num_heads = config.MODEL.SWIN.NUM_HEADS
        self.window_size = config.MODEL.SWIN.WINDOW_SIZE
        self.mlp_ratio = config.MODEL.SWIN.MLP_RATIO
        self.qkv_bias = config.MODEL.SWIN.QKV_BIAS
        self.qk_scale = config.MODEL.SWIN.QK_SCALE
        self.drop_rate = config.MODEL.DROP_RATE
        self.attn_drop_rate = 0.
        self.drop_path_rate = config.MODEL.DROP_PATH_RATE
        self.ape = config.MODEL.SWIN.APE
        self.patch_norm = config.MODEL.SWIN.PATCH_NORM
...
class SwinTransformerForSimMIM(SwinTransformer):
    ...
    def forward(self, x, mask=None):
        ...
        # forward and output all level feature
        hw = [H, W]
        features_list = []
        hws_list = []
        stages_list = []
        ...
        for layer in self.layers:
            ...
            stage2last_f_list[stage] = x

            x, next_x = layer(next_x)
            H = W = int(x.shape[1] ** 0.5)
            hw_new = [H, W]
            if hw_new != hw or stage == 0:
                stage += 1
                stage2last_f_list.append(0)
            hw = hw_new
        ...
        return {'features': features_list, 'hws': hws_list, 'stages': stages_list, 'stage_last_feat': stage2last_f_list}
