...
from lpe.utils.utils import show_attention_relation_t2v
...
class BeitSelfAttention(nn.Module):
    def __init__(self, config: BeitConfig, is_cross_attention, window_size: Optional[tuple] = None) -> None:
        super().__init__()
        ...
        if is_cross_attention:
            self.key = nn.Linear(config.encoder_width, self.all_head_size, bias=False)
...
class BeitAttention(nn.Module):
    def __init__(self, config: BeitConfig, is_cross_attention=False, window_size: Optional[tuple] = None) -> None:
        super().__init__()
        self.attention = BeitSelfAttention(config, is_cross_attention=is_cross_attention, window_size=window_size)
...
class BeitLayer(nn.Module):
    ...
    def __init__(self, config: BeitConfig, layer_num, window_size: Optional[tuple] = None, drop_path_rate: float = 0.0) -> None:
        ...
        self.has_cross_attention = (layer_num in range(config.fusion_start, config.fusion_end))
        if self.has_cross_attention:
            self.crossAttention = BeitAttention(config, is_cross_attention=True, window_size=window_size)
    ...
    def forward(
        ...
        # cross attention
        if self.has_cross_attention:
            assert encoder_hidden_states is not None, "BEiT: encoder_hidden_states must be given for cross-attention layers"
            if type(encoder_hidden_states) == list:
                cross_attention_outputs = self.crossAttention(
                    self.layernorm_before(hidden_states),
                    head_mask,
                    encoder_hidden_states[(self.layer_num - self.config.fusion_start) % len(encoder_hidden_states)],
                    output_attentions=output_attentions,
                )
                attention_output = cross_attention_outputs[0]
                outputs = outputs + cross_attention_outputs[1:]  # add self attentions if we output attention weights
...
class BeitEncoder(nn.Module):
    ...
    def forward(
        ...
        if mode == 'image':
            layer_list = list(range(self.config.image_start, self.config.image_end))
        elif mode == 'fusion':
            layer_list = list(range(self.config.fusion_start, self.config.fusion_end))
        else:
            layer_list = list(range(self.config.image_start, self.config.image_end)) + \
                         list(range(self.config.fusion_start, self.config.fusion_end))
...
class BeitModel(BeitPreTrainedModel):
    ...
    def forward(
        ...
        if pixel_values is None and encoder_embeds is None:
