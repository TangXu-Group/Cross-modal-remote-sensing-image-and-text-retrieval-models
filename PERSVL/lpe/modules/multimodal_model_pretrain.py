...
from lpe.utils.utils import load_model, show_attention_relation_mv
...
class CRSRT(nn.Module):
    ...
    def __init__(self, args, swin_cfg, tokenizer):
        ...
        self.beta_rate = nn.Parameter(torch.tensor(1.), requires_grad=True)
        self.itm_head = nn.Linear(text_embed_dim, 2)

    def forward(self, image, text, idx_mask, noun_idxs=None, img_mask=None, orgin_text=None, epoch=None):
        ...
        if self.args.use_mm_fusion is False:
            loss_i2t = F.cross_entropy(sim_i2t, torch.arange(len(sim_i2t)).to(sim_i2t.device))
            loss_t2i = F.cross_entropy(sim_t2i, torch.arange(len(sim_t2i)).to(sim_t2i.device))

            loss_itc = (loss_i2t + loss_t2i) / 2

            return loss_itc, None, None, None, None, None
        ...
        itm_labels = torch.cat([torch.ones(bs, dtype=torch.long), torch.zeros(2 * bs, dtype=torch.long)],
                               dim=0).unsqueeze(1).to(image.device).float()
        itm_targets = torch.cat([1 - itm_labels, itm_labels], dim=1)
        loss_itm = -torch.sum(F.log_softmax(vl_output, dim=1) * itm_targets, dim=1).mean()
        ...
        if self.args.use_mlm:
            ...
            #         masked_indices[each][noun_idx] = True
            #         # choose a object that will not be masked to make a better interaction between image and text
            #         if len(noun_idx) > 1:
            #             non_mask_idx_idx = np.random.choice(range(len(noun_idx)), size=1)
            #             masked_indices[each][noun_idx[non_mask_idx_idx[0]]] = False
            #
            #     input_ids, labels = self.mask(input_ids, self.text_encoder.config.vocab_size, image.device, targets=labels,
        ...
        return loss_itc, loss_itm, loss_mlm, loss_mim, image_idxs_neg, text_idxs_neg
    ...
    def fusion_forward(self, text_embeds, text_attn, image_embeds, image_attn):
        ...
        if vl_embeddings_i2t is None:
            vl_embeddings_i2t = vl_embeddings_t2i
        if vl_embeddings_t2i:
            vl_embeddings_t2i = vl_embeddings_i2t
    ...
    @torch.no_grad()
    def m_forward(self, image, text, img_neg_idxs, txt_neg_idxs):
