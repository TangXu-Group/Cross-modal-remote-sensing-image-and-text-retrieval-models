...
#         # print("this is the shape of input x:{}".format(x.shape))
#         value = self.fc_v(x)
#         x = self.fc1(x)
#         x = rearrange(x, "b (e c) w h -> (b e) c w h", e=self.heads)
#         value = rearrange(value, "b (e c) w h -> (b e) c w h", e=self.heads)
#         if self.fold_w>1 and self.fold_h>1:
#             # split the big feature maps to small local regions to reduce computations of matrix multiplications.
#             b0,c0,w0,h0 = x.shape
...
#             self.head = nn.Linear(in_features=256, out_features=768)
#
...
#
#         ckpt_path = "/data1/amax/wyj/Cocs/epoch_12.pth"
...
#         proposal_w=proposal_w, proposal_h=proposal_h, fold_w=fold_w, fold_h=fold_h,
#         heads=heads, head_dim=head_dim,pretrained=pretrained,
#         **kwargs)
...
#
#
# def cluster_model(pretrained=False, **kwargs):
#     layers = [1, 1]
#     norm_layer=GroupNorm
#     embed_dims = [128, 256]
#     mlp_ratios = [8, 8]
...
default_cfgs = {
    'model_small': _cfg(crop_pct=0.9),
    'model_prior': _cfg(crop_pct=1.0, num_classes=768),
    'model_medium': _cfg(crop_pct=0.95),
}
...
class Cluster(nn.Module):
    ...
    def forward(self, x):  # [b,c,w,h]
        value = self.v(x)                                                                                               # map points to value space P_{v}
        x = self.f(x)                                                                                                   # map points to P_{s} for similarity computation P_{s}^{'}
        x = rearrange(x, "b (e c) w h -> (b e) c w h", e=self.heads)                                             # manage P_{s}^{'} to computer multi-head
        value = rearrange(value, "b (e c) w h -> (b e) c w h", e=self.heads)                                     # manage P_{v} to computer multi-head
        if self.fold_w > 1 and self.fold_h > 1:
            # split the big feature maps to small local regions to reduce computations.
            b0, c0, w0, h0 = x.shape
            assert w0 % self.fold_w == 0 and h0 % self.fold_h == 0, \
                f"Ensure the feature map size ({w0}*{h0}) can be divided by fold {self.fold_w}*{self.fold_h}"
            x = rearrange(x, "b c (f1 w) (f2 h) -> (b f1 f2) c w h", f1=self.fold_w,                               # split points
                          f2=self.fold_h)  # [bs*blocks,c,ks[0],ks[1]]
            # print("this is the shape of x_!:{}".format(x.shape))
            value = rearrange(value, "b c (f1 w) (f2 h) -> (b f1 f2) c w h", f1=self.fold_w, f2=self.fold_h)        # split value points
            # print("this is the shape of value_!:{}".format(value.shape))
        b, c, w, h = x.shape
        centers = self.centers_proposal(x)  # [b,c,C_W,C_H], we set M = C_W*C_H and N = w*h                                 # obtain centers for P_{s}^{'} with multi-head
        value_centers = rearrange(self.centers_proposal(value), 'b c w h -> b (w h) c')  # [b,C_W,C_H,c]             # obtain centers for P_{v} with multi-head
        b, c, ww, hh = centers.shape
        sim = torch.sigmoid(
            self.sim_beta +
        ...
        # print("this is the shape of sim:{}".format(sim.shape))# calculate similarity
        # we use mask to sololy assign each point to one center
        sim_max, sim_max_idx = sim.max(dim=1, keepdim=True)                                                                 # obtain similarity information
        mask = torch.zeros_like(sim)  # binary #[B,M,N]
        mask.scatter_(1, sim_max_idx, 1.)
        sim = sim * mask                                                                                                     # define right
        value2 = rearrange(value, 'b c w h -> b (w h) c')  # [B,N,D]
        # print("this is the shape of value2:{}".format(value2.shape))
        # aggregate step, out shape [B,M,D]
        out = ((value2.unsqueeze(dim=1) * sim.unsqueeze(dim=-1)).sum(dim=2) + value_centers) / (                            # out
        ...
        if self.fold_w > 1 and self.fold_h > 1:
            ...
            out = rearrange(out, "(b f1 f2) c w h -> b c (f1 w) (f2 h)", f1=self.fold_w, f2=self.fold_h)
            # print("this is the shape of out2:{}".format(out.shape))
        out = rearrange(out, "(b e) c w h -> b (e c) w h", e=self.heads)
        # print("this is the shape of out3:{}".format(out.shape))                                 # out
        out = self.proj(out)
        # print("this is the shape of out4:{}".format(out.shape))                                                                                              # out MLP
        return out
    ...
    def forward(self, x):
        if self.use_layer_scale:
            ...
            x = x + self.drop_path(
                self.layer_scale_2.unsqueeze(-1).unsqueeze(-1)
                * self.mlp(self.norm2(x)))
            # print("this is self.layer_scale_1:{} // self.layer_scale_2:{}".format(self.layer_scale_1, self.layer_scale_2))
        else:
            x = x + self.drop_path(self.token_mixer(self.norm1(x)))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x
...
def cluster_model(pretrained=False, num_classes=512,  **kwargs):
    layers = [1, 1]
    norm_layer=GroupNorm
    embed_dims = [128, 256]
    mlp_ratios = [8, 8]
...
#         heads=heads, head_dim=head_dim,
#         **kwargs)
#     model.default_cfg = default_cfgs['model_prior']
#     return model


if __name__ == '__main__':
    input = torch.rand(2, 3, 224, 224)
    model = cluster_model(num_classes=768)
    out = model(input)
    print(model)
    print(out.shape)
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print("number of params: {:.2f}M".format(n_parameters/1000**2))
