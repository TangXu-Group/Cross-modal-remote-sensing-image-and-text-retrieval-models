...
class MTDataModule(LightningDataModule):
    ...
    def setup(self, stage):
        ...
        self.collate = functools.partial(
            self.dms[0].train_dataset.collate, mlm_collator=None,
        )

        if self.dist:

            # # ShardedDataParallel
            # self.train_sampler = ShardedDataParallel(self.train_dataset, shuffle=True)
            # self.val_sampler = ShardedDataParallel(self.val_dataset, shuffle=True)
            # self.test_sampler = ShardedDataParallel(self.test_dataset, shuffle=False)
