...
from vilt.modules.objectives import compute_clip_recall
    ...
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                setattr(pl_module, f"{split}_match_loss", Scalar())
                setattr(pl_module, f"{split}_uncertainty_loss", Scalar())
                setattr(pl_module, f"{split}_cls_loss", Scalar())
                setattr(pl_module, f"{split}_prior_loss", Scalar())
...
def epoch_wrapup(pl_module):
    ...
    if pl_module.hparams.config["get_recall_metric"] and not pl_module.training:
        ...
        # print("################  Image Classification Evaluation Metrics  ################")
        # for k, v in img_cls.items():
        #     img_cls[k] = round(100*v, 2)
        # print(
        #     "Image precision score: {}%".format(img_cls["precision_score"]),
        #     ", image balanced accuracy score: {}%".format(img_cls["balanced_accuracy_score"]),
        #     ", image f1 score: {}%".format(img_cls["f1_score"]),
        #     ", image recall score: {}%".format(img_cls["recall_score"]),
        #     pl_module.global_step)
        # print("\n")
        #
        # print("################  Text Classification Evaluation Metrics  ################")
        # for k, v in txt_cls.items():
        #     txt_cls[k] = round(100*v, 2)
        # print(
        #     "Text precision score: {}%".format(txt_cls["precision_score"]),
        #     ", text balanced accuracy score: {}%".format(txt_cls["balanced_accuracy_score"]),
        #     ", text f1 score: {}%".format(txt_cls["f1_score"]),
        #     ", text recall score: {}%".format(txt_cls["recall_score"]),
        #     pl_module.global_step)
        # print("\n")
        ...
        pl_module.logger.experiment.add_scalar(
            "recalls/mean", mean, pl_module.global_step
        )

        pl_module.logger.experiment.add_scalar(
            "recalls/iir_r1", iir_r1, pl_module.global_step
        )
        pl_module.logger.experiment.add_scalar(
            "recalls/iir_r5", iir_r5, pl_module.global_step
        )
        pl_module.logger.experiment.add_scalar(
            "recalls/iir_r10", iir_r10, pl_module.global_step
        )
        pl_module.logger.experiment.add_scalar(
            "recalls/ttr_r1", ttr_r1, pl_module.global_step
        )
        pl_module.logger.experiment.add_scalar(
            "recalls/ttr_r5", ttr_r5, pl_module.global_step
        )
        pl_module.logger.experiment.add_scalar(
            "recalls/ttr_r10", ttr_r10, pl_module.global_step
        )
        pl_module.logger.experiment.add_scalar(
            "recalls/im_mean", im_mean, pl_module.global_step
        )
        the_metric += im_mean
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            pl_module.log(
                f"{loss_name}/{phase}/clip_loss",
                getattr(pl_module, f"{phase}_clip_loss").compute(),
            )
            getattr(pl_module, f"{phase}_clip_loss").reset()
            # pl_module.log(
            #     f"{loss_name}/{phase}/irtr_kl_loss",
            #     getattr(pl_module, f"{phase}_kl_loss").compute(),
            # )
            # getattr(pl_module, f"{phase}_kl_loss").reset()

            # pl_module.log(
            #     f"{loss_name}/{phase}/irtr_uncertainty_loss",
            #     getattr(pl_module, f"{phase}_uncertainty_loss").compute(),
            # )
            # getattr(pl_module, f"{phase}_uncertainty_loss").reset()
            #
            pl_module.log(
                f"{loss_name}/{phase}/prior_loss",
                getattr(pl_module, f"{phase}_prior_loss").compute(),
            )
            getattr(pl_module, f"{phase}_prior_loss").reset()
            # #
            # pl_module.log(
            #     f"{loss_name}/{phase}/irtr_cls_loss",
            #     getattr(pl_module, f"{phase}_cls_loss").compute(),
            # )
            # getattr(pl_module, f"{phase}_uncertainty_loss").reset()
            pl_module.log(
                f"{loss_name}/{phase}/uncertainty_loss",
                getattr(pl_module, f"{phase}_uncertainty_loss").compute(),
            )
            getattr(pl_module, f"{phase}_uncertainty_loss").reset()
...
def set_schedule(pl_module):
    ...
    # prompt_weight = pl_module.visual_prompt_weight
    ...
    optimizer_grouped_parameters = [
        {
            "params": [
                ...
                if not any(nd in n for nd in no_decay)
                   ...
                   and "visual_prompt_weight" not in n
                # and not any(pb in n for pb in prompt_weight)
            ],
            "weight_decay": wd,
            "lr": lr,
        },
